...
class M_member extends CI_Model
{
    ...
    {
        $id = $this->session->userdata('id');
        return $this->db->query("SELECT w.idWishlist, p.idProduk, p.namProduk, p.foto, p.harga FROM tbl_wishlist w JOIN tbl_produk p ON w.idProduk=p.idProduk JOIN tbl_member m ON w.idKonsumen=m.idKonsumen WHERE w.idKonsumen=$id
        ");
        //$q = $this->db->get_where('tbl_wishlist', array('idKonsumen' => $id));
        //return $q;
    }
